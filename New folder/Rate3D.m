...
[alpha_array,Psi_ps_array] = meshgrid(0:0.05:1,0:0.05:1);
size_2D = size(Psi_ps_array);
size_1D = size_2D(1);
rate_Analytical_ub_apx = zeros(size_1D,size_1D);
rate_sim_min = zeros(size_1D,size_1D);
Tc = 1; %coherence time
...
for kk=1:size_1D
    %% variables
    alpha = alpha_array(1,kk); %TS factor
    for xx = 1:size_1D
    Psi_ps = Psi_ps_array(xx,1); %power scaling factor
    P=10; %logspace(-1,3,20); 
%     P=linspace(0,30000,30); %transmitted  power
...
%          power=eff*Ptx*alpha*Tc*(G_opt_q.*alpha_f.*amp1)^2+eff*Ptx*(1-alpha)*Tc*(G_opt_q.*alpha_f.*amp2)^2;
...
%     quantized_power = eff*Ptx*alpha*Tc*(abs(G_opt_q.*quantized_amplitudes1).*amp1_q)^2+eff*Ptx*(1-alpha)*Tc*(G_opt_q.*alpha_f.*quantized_amplitudes2)^2;
%     
...
%==========================================================================  
    ...
    average_snr_min=Psi_ps*gamma_star_count_min/NS;    
...
% rate calculation---------------------------------------------------------
    ...
    rate_sim_min(kk,xx)= alpha*log2(1+average_snr_min);
...
% % %     rate_Analytical_ub_int=[rate_Analytical_ub_int log2(1+average_snr_ana_ub_int)]; %try
    ...
    rate_Analytical_ub_apx(kk,xx)= alpha*log2(1+Psi_ps*average_snr_ana_ub_apx);  
...
% plot(P,quantized_Eh_vec,'o','color',[1 0 1],'LineWidth',2,'MarkerSize',8); hold on;
% plot(P,quantized_Eh_nl,'-','color',[0 0 0],'LineWidth',2,'MarkerSize',8); hold on;
...
% plot(gamma_bar_db,rate_sim_ub,':s','color',[0 0 1],'LineWidth',2,'MarkerSize',10);hold on;
% surf(X,Y,Z)
surf(alpha_array,Psi_ps_array,rate_sim_min);hold on;
% plot(gamma_bar_db,rate_Analytical_ub_int,'-','color',[1 0.5 1],'LineWidth',2);hold on;
surf(alpha_array,Psi_ps_array,rate_Analytical_ub_apx);