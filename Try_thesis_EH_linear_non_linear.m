...
eff = 1;%0.5; % Energy harvesting efficiency

bit_array = [inf]; %[1 2 4];

delta= 9.079; omega= 2.9 ;psi=47.083*1e-3; %psi =a, d=b=omega, delta = K
...
%==========================================================================       
        amp1_d=sum(eta.*alpha_h.*alpha_g,2); % breaking the expression into two parts and 1st part has amplitude of amp1
        amp2_d=sqrt(Psi_ps)*amp1_d; % second amplitude has powerscaling factor in it
%         G_opt_d = sqrt(gamma_bar(pp)./(gamma_bar(pp).*(amp1_d).^2+1));
G_opt_d =1;
        
        power_d=eff*Ptx*alpha*Tc*(G_opt_d.*alpha_f.*amp1_d)^2+eff*Ptx*(1-alpha)*Tc*(G_opt_d.*alpha_f.*amp2_d)^2;
        
        Rate_count=Rate_count+power_d;
        power1=Ptx*(sum(amp1_d).*G_opt_d.*alpha_f)^2 ;
        power2=Ptx*sqrt(theta)*(sum(amp2_d).*G_opt_d.*alpha_f)^2;
...
Eh_nl = alpha*Tc*tmp1 + (1-alpha)*Tc*tmp1;
%==========================================================================  
...
% figure(7)
% plot(Eh_sim_exct,rate_sim_exct,'--o','color',[1 0 1],'LineWidth',2,'MarkerSize',9);hold on;
% 
% plot(Eh_Analytical,rate_Analytical_ub_int,'-','color',[1 0.5 1],'LineWidth',2);hold on;
% plot(Eh_Analytical,rate_Analytical_ub_apx,'--','color',[0 0.5 0],'LineWidth',2);

end
figure(10);
gammabardB=10*log10(P);

plot(P,Rate_vec,'o','color',[0 0 1],'LineWidth',2); hold on;
plot(P,Eh,'--','color',[1 0 0],'LineWidth',2); hold on;
plot(P,Eh_vec,'o','color',[0 0 1],'LineWidth',2); hold on;
plot(P,Eh_nl,'-','color',[1 0 0],'LineWidth',2); hold on;
